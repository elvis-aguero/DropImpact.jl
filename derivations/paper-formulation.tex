...
Two consequences follow, and neither is a statement about compactness. First, the pressure is resolvable pointwise provided $N$ stays inside the budget: refining $N$ at fixed $M,L$ cannot converge, since it adds trial directions the test space cannot see, whereas joint refinement does converge. At $N=12$, $\theta_c=0.3$ and fixed basis, $\mathrm{cond}$ of the inner Galerkin matrix falls from $5.0\times10^{10}$ at $M=L=80$ to $32$ at $M=L=640$, with nothing about the pressure representation changed. Second, the ill-conditioning is not driven by the time step: the plateau dimension is unchanged (at $13$) across $\delta\in[10^{-4},1]$, while $\sigma_1$ itself moves five orders of magnitude over the same range. Indeed at production step sizes the compliance symbol is monotonically \emph{increasing} over the whole retained spectrum -- the bath and droplet respond inertially, with symbols growing like $k$ and $l^2$ respectively, turning over only near $k_\ast=(\mathfrak a/\delta)^{2/3}\approx131$ at $\delta=10^{-3}$, well above $k_M\approx42$ at $M=80$ -- so the retained operator is anti-smoothing rather than smoothing, and first-kind-Fredholm intuition does not describe it.

Practically, then, $N$ should be provisioned against the budget rather than upward for safety margin: over-provisioning $N$ at fixed $M,L$ is what breaks the solve. Only the low-order moments $c_m,b_l,f$ enter the governing equations \eqref{eq:bath-mode}, \eqref{eq:drop-mode}, \eqref{eq:com}, and those remain well conditioned even past the budget, since they pair the pressure against smooth test functions; the pointwise pressure does not, and its positivity check is correspondingly a diagnostic of whether the budget is being respected.
