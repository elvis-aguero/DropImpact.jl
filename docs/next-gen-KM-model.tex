...
\end{equation}
The $J_0(k_mb)$ factors cancel out of $D$ entirely. Since $\kappa_m<0$ for every $m$ the sum cannot cancel, and it converges because $\kappa_m\sim-2/k_m^2$; the elimination leaves $a_m$ affine in $c$ with a rank-one correction to the bath compliance, so the inner Galerkin system stays square and linear in the same unknowns and the nested closure of \S\ref{subsec:contact} is untouched, at a cost of one $O(M)$ dot product. Two further properties matter. The rank-one correction is self-adjoint in the same weighted pairing $\langle\cdot,\cdot\rangle_w$, $w_m\propto J_0(k_mb)^2$, that \S\ref{subsubsec:compliance} uses, so it does not disturb the self-adjointness the formulation was built around. And $\kappa_0=0$, so the rim force cannot drive the piston mode: pinning by constraint conserves volume exactly, and does not reintroduce Route A's defect.
...
Route A is what the implementation currently provides, as a non-default option (\texttt{wall=:pinned}), and it should be understood as a diagnostic rather than a physical model: pinning is exact and sustained --- $|\eta(b,\tau)|$ stays at roundoff throughout an impact, and both weights agree with $1/\lVert J_0(k_m\cdot)\rVert^2$ to better than $10^{-15}$ --- but volume is not conserved, by the margin quoted above. Route B is the formulation consistent with \eqref{eq:bath-pde-1}--\eqref{eq:bath-pde-2}, and is the recommended route; it is not yet implemented. Two scope limitations should be recorded. The pinned option alters the bath weight and deletes the piston mode, hence alters the compliance operator, so every diagnostic in \S\ref{subsec:contact} and \S\ref{subsubsec:compliance} --- self-adjointness, semidefiniteness, conditioning, the tangency root --- is a free-wall measurement that has not been repeated under \texttt{:pinned}. And the difference between the two walls in contact time and restitution coefficient is \emph{not resolved} at the tolerances used here: it is smaller than the spread produced by legitimate changes to the step controller, so no directional claim about a pinned bath being stiffer or softer is supported.
