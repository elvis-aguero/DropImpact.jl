...
\end{equation}
whence the weight $2/(bJ_0(k_mb))^2$ used above. \cite{AlventosaEtAl2023}'s eq.~(300) instead prints $2/(bJ_1(k_m))^2$, which is the normalizer for a \emph{Dirichlet} basis ($J_0(k_mb)=0$, $k_mb$ a zero of $J_0$) and moreover evaluates $J_1$ at $k_m$ rather than $k_mb$. Under the no-flux condition they themselves impose ($\partial_n\phi=0$ on the walls, hence $\partial_r\eta=0$ at $r=b$), $J_1(k_mb)$ vanishes identically, so their expression is inconsistent with their own basis. The discrepancy is mode-dependent and locally severe rather than a uniform rescaling: evaluated at $b=6$ against \eqref{eq:bessel-norm}, the ratio of their weight to the correct one ranges over roughly $0.16$ to $1.8$ for $m=1,\dots,8$ but reaches $116$ at $m=7$, because $k_7=3.7933$ falls close to the first zero of $J_1$ at $3.8317$, driving $J_1(k_m)\to0$ and their weight to diverge. An earlier revision of this document printed that ratio list inverted, which contradicted this very explanation --- a diverging $J_1(k_m)^{-2}$ cannot produce a near-zero ratio in the stated orientation. An earlier revision of this document reproduced their expression verbatim; every bath-side quantity reported here uses \eqref{eq:bessel-norm}.

\subsubsection{Wall condition: a free versus a pinned contact line}
\label{subsubsec:wall}

Everything above assumes the bath meets the container wall at a free, $90^\circ$ contact line: $\partial_r\eta=0$ at $r=b$, the same condition the flow obeys. A second experimental configuration is of interest --- a finite container with no-flux walls in which the free surface is \emph{pinned} at the triple point, $\eta(b,\tau)=0$ with the wall slope free, as a real meniscus anchored on a machined rim behaves. The two conditions are not compatible with a single Fourier--Bessel set, and the incompatibility is structural rather than a matter of resolution.

The reason is not a limitation of any basis. It is a property of the continuum problem, and it is worth deriving because it determines which pinned models are admissible at all. Exact no-flux on a vertical wall means $\partial_r\phi(b,z)=0$ for every $z\in[-h_0,0]$; differentiating in $z$ gives $\partial_r\partial_z\phi(b,0)=0$, and the kinematic condition \eqref{eq:bath-pde-1} then yields
\begin{equation}
...
\end{equation}
In a rigid right-cylindrical container with no-flux walls the free surface's slope at the wall is therefore \emph{frozen}, to $O(\Oh)$, at whatever value it starts with. The consequence is sharper than an inconvenience: ``pinned at the rim with a freely time-varying wall slope'' is not a solution of \eqref{eq:bath-pde-1}--\eqref{eq:bath-pde-2} under no-flux walls at all --- it is over-determined. The admissible pinned configuration is the \emph{clamped edge}, $\eta(b)=0$ together with $\partial_r\eta(b)$ frozen, and a constant nonzero wall slope is accommodated by splitting $\eta=\eta_s(r)+\sum_ma_m(\tau)J_0(k_mr)$ with $\eta_s$ a static meniscus satisfying $\eta_s(b)=0$, $\eta_s'(b)\neq0$ and the dynamic part on the no-flux set. Pinning, no-flux and a nonzero wall slope are then all exact simultaneously. (The pinned-edge eigenvalue problem in a rigid container is classical: see \citealt{Scott1979} and \citealt{GrahamEagle1983}. \citealt{Hocking1987}'s Robin edge condition, of the form $kJ_1(kb)=\lambda J_0(kb)$, interpolates between the free and pinned limits as $\lambda$ runs from $0$ to $\infty$ and is a further option not pursued here.)

Against that background the two discrete routes are as follows. The no-flux set $\{J_0(k_mr):J_1(k_mb)=0\}$ used above has $\partial_r\eta(b)=0$ for every member; the Dirichlet set $\{J_0(k_mr):J_0(k_mb)=0\}$ has $\eta(b)=0$ for every member with the wall slope free, which is pinning as an identity of the basis, but its radial velocity at the wall goes like $J_1(k_mb)$, which does not vanish.

\emph{Route A: the Dirichlet basis.} Take $k_mb$ to be the $m$-th zero of $J_0$. Pinning is then exact mode by mode --- an identity of the basis, not a constraint to be enforced --- and the wall slope is unconstrained. Two consequences follow. First, the squared norm \eqref{eq:bessel-norm} becomes
\begin{equation}
    \int_0^bJ_0(k_mr)^2\,r\,dr = \frac{b^2}{2}\Big[J_0(k_mb)^2+J_1(k_mb)^2\Big] = \frac{b^2}{2}J_1(k_mb)^2\text{,}
    \label{eq:bessel-norm-pinned}
\end{equation}
so the Fourier--Bessel weight in \eqref{eq:c_m-def} is $2/(bJ_1(k_mb))^2$. This is the Dirichlet normalizer whose form \cite{AlventosaEtAl2023} print --- modulo the second, independent error \S\ref{subsec:bath} identifies, since they evaluate $J_1$ at $k_m$ rather than at $k_mb$, and $b=6$ here. It is the right normalizer for a pinned bath applied to a free one, which is part of why the slip was easy to make; the two configurations also differ in the eigenvalues themselves and in the presence of the $k=0$ mode, so the substitution is not the whole of the difference.

Second, the piston mode disappears. The zeros of $J_0$ begin at $k_1b=2.4048$, so there is no $k=0$ member, whereas the no-flux set begins with $k_0=0$ and needs it. This is not a convention: a surface pinned on its entire boundary cannot translate uniformly, so the uniform mode is not an eigenfunction of the pinned problem and its absence is the correct physics.

The cost is severe, and stating it as ``a leaking wall'' understates it. What the Dirichlet basis loses is \emph{exact conservation of bath volume}, which the no-flux basis possesses structurally. For the no-flux set every non-piston mode is volume-neutral, since
\begin{equation}
    \int_0^bJ_0(k_mr)\,r\,dr = \frac{b}{k_m}J_1(k_mb) = 0\qquad\text{whenever }J_1(k_mb)=0\text{,}
    \label{eq:volume-neutral}
\end{equation}
and the only volume-carrying mode is the piston, whose pressure response $\kappa_0$ vanishes identically, so it can never be driven. Displaced volume is thus conserved by construction rather than by accuracy. For the Dirichlet set \eqref{eq:volume-neutral} fails for every mode and nothing constrains the sum. Measured over the reference impact, $\big|\int_0^b\eta\,r\,dr\big|$ stays at $5\times10^{-15}$ under \texttt{:free} but reaches $2.79$ under \texttt{:pinned} --- $17.5$ droplet radii cubed of bath volume created from nothing, four times the droplet's own volume of $4\pi/3$. For a model whose entire purpose is to transfer displaced volume from droplet to bath and radiate it as capillary waves, that is disqualifying, not a caveat.

\emph{Route B: the no-flux basis with a constraint.} Retain the Neumann set, so that no-flux and \eqref{eq:volume-neutral} both hold identically, and impose pinning as the single scalar constraint $\mathcal C(a)=\sum_ma_mJ_0(k_mb)=0$ carried by a multiplier $\Lambda$ --- physically the line force the rim exerts on the contact line, which is a quantity of interest in its own right rather than an artifice. The multiplier enters as a line force $p_\Lambda=\Lambda\,\delta(r-b)$, whose Fourier--Bessel coefficient under \eqref{eq:c_m-def} is $c_m^\Lambda=2\Lambda/(bJ_0(k_mb))$, taking $\int_0^bf(r)\delta(r-b)\,dr=f(b)$ at the domain's own boundary --- a convention, since a delta sitting exactly at an endpoint admits no unique weight, but a harmless one, as any other choice only rescales $\Lambda$ itself rather than changing the structure below. So \eqref{eq:kappa-m} gains one term and the constraint closes on
\begin{equation}
    a_m = \alpha_m + \kappa_mc_m + \frac{2}{b}\frac{\kappa_m\Lambda}{J_0(k_mb)}\text{,}\qquad
    D = \frac{2}{b}\sum_m\kappa_m\text{.}
    \label{eq:route-b-multiplier}
\end{equation}
The $J_0(k_mb)$ factors cancel out of $D$ entirely. Since $\kappa_m<0$ for every non-piston mode (the sole exception, $\kappa_0=0$, contributes nothing to the sum either way) the sum cannot cancel, and it converges because $\kappa_m\sim-2/k_m^2$; the elimination leaves $a_m$ affine in $c$ with a rank-one correction to the bath compliance, so the inner Galerkin system stays square and linear in the same unknowns and the nested closure of \S\ref{subsec:contact} is untouched, at a cost of one $O(M)$ dot product. Two further properties matter. The rank-one correction is self-adjoint in the same weighted pairing $\langle\cdot,\cdot\rangle_w$, $w_m\propto J_0(k_mb)^2$, that \S\ref{subsubsec:compliance} uses --- measured at relative asymmetry $1.1\times10^{-17}$, matching that section's own $10^{-16}$ figure for the free operator, not merely asserted --- so it does not disturb the self-adjointness the formulation was built around. And $\kappa_0=0$, so the rim force cannot drive the piston mode: pinning by constraint conserves volume exactly, and does not reintroduce Route A's defect.

Three further facts, assembled and checked at operator level rather than argued in isolation (\texttt{derivations/feasibility\_pinned\_contact\_line.jl}, \S5), close the remaining gaps between the algebra above and an implementation. First, $J_0(k_mb)$ never vanishes on the Neumann set: $J_0'=-J_1$, so a common zero of $J_0$ and $J_1$ would force $J_0$ to vanish to second order there, which for a solution of Bessel's equation forces $J_0\equiv0$ --- false. $|J_0(k_mb)|$ does decay with $m$, but along the generic $m^{-1/2}$ Bessel envelope rather than toward a near-miss with a zero, and the quantity that actually appears in $a_m$, $\kappa_m/J_0(k_mb)$, falls as $m^{-3/2}$ and saturates (its maximum over $m$ stops growing by $M\sim640$): no per-mode term is amplified by refining the truncation. Second, the constraint is an identity of the assembled $(M{+}1)\times(M{+}1)$ operator $\mathcal A_{\text{clamped}}$, not a property of one solved-for $c$: $\sum_m(\mathcal A_{\text{clamped}})_{mn}J_0(k_mb)=0$ for every column $n$, measured at $5\times10^{-22}$. Third, the sign is preserved, and not merely at the one parameter point checked above: writing $-W\mathcal A_{\text{clamped}}=\mathrm{diag}(-\kappa_mJ_0(k_mb)^2)+\big(\tfrac{-2}{bD}\big)zz^\top$ with $z_m=\kappa_mJ_0(k_mb)$, both terms are positive semidefinite given only $\kappa_m\le0$ for every mode and $D<0$ --- and those two facts hold for \emph{any} admissible step, since $\mathfrak a=(1+2s)/(1+s)>0$ for every BDF2 step ratio $s>0$ and $\Oh,\Bo\ge0$ in $\kappa_m$'s closed form. The rank-one term therefore cannot push $-\mathcal A_{\text{clamped}}$ out of positive semidefiniteness at any step, not only the representative one measured numerically above.

Route B's representational limitation is far weaker than an earlier revision of this section claimed. Expanding the pinned target $\eta=J_0(j_{0,1}r/b)$ on the Neumann set, the truncation error computed analytically by Parseval falls as $M^{-3/2}$, and the wall slope is reproduced pointwise at every $r<b$; only at the single point $r=b$ does the truncated sum have zero derivative, with a boundary layer of width $\sim b/k_M$ and a bounded Gibbs overshoot of a few percent. The failure is confined to a set of measure zero. Given \eqref{eq:wall-slope-frozen} there is in any case no time-varying wall slope to represent.

That earlier revision reached the opposite conclusion from a diagnostic that was measuring its own grid. It finite-differenced the fitted profile across the last interval of a fixed $4000$-point grid, i.e.\ \emph{inside} the boundary layer, so the number returned scales as $\Delta r\,k_M$ rather than as any representation error: holding $M=640$ fixed and refining the grid drives it from $-0.033$ to $-0.0005$, toward zero, while the true slope is $-0.208$. The suspiciously clean doubling of that ``slope'' with each doubling of $M$ was the signature of the artifact. The relative $L^2$ column was likewise quadrature-limited and could not discriminate. Both are corrected in \texttt{derivations/feasibility\_pinned\_contact\_line.jl}, which now differentiates the series analytically at fixed $r<b$.

Route A is what the implementation provides as a non-default diagnostic option (\texttt{wall=:pinned}), not a physical model: pinning is exact and sustained --- $|\eta(b,\tau)|$ stays at roundoff throughout an impact, and both weights agree with $1/\lVert J_0(k_m\cdot)\rVert^2$ to better than $10^{-15}$ --- but volume is not conserved, by the margin quoted above. Route B, the recommended formulation consistent with \eqref{eq:bath-pde-1}--\eqref{eq:bath-pde-2}, is implemented as \texttt{wall=:clamped}, applying \eqref{eq:route-b-multiplier} at every step, including free flight (pinning is a constraint on $a_m$ itself, not a byproduct of contact pressure). Measured over the same reference impact, \texttt{:clamped} holds both quantities at roundoff simultaneously: $|\eta(b,\tau)|$ at $7\times10^{-17}$ and $\big|\int_0^b\eta\,r\,dr\big|$ at $2\times10^{-14}$, where \texttt{:pinned} traded one for the other. Two scope limitations should be recorded. \texttt{:clamped} reuses the \texttt{:free} basis and weight exactly, and \S5 of \texttt{derivations/feasibility\_pinned\_contact\_line.jl} verifies the corrected response operator's self-adjointness and sign at the operator level (\S\ref{subsubsec:wall} above), but the step-level diagnostics of \S\ref{subsec:contact} and \S\ref{subsubsec:compliance} --- conditioning of the assembled nonlinear system, the resolvable pressure dimension, the tangency root --- are free-wall measurements that have not been repeated for \texttt{:clamped} at a real contact step. The difference between \texttt{:free} and \texttt{:clamped} is now measurable rather than an artifact, since both conserve volume, and it is strongly $b$-dependent: at the production default $b=6$ it is negligible ($\Delta\mathrm{CoR}=-0.001$), but shrinking the container moves it far outside step-controller noise and it is \emph{not} monotone in $b$ --- $\Delta\mathrm{CoR}=+0.26$ at $b=4$, $+0.11$ at $b=3$, $+0.07$ at $b=2$, $-0.21$ at $b=1.5$ --- consistent with a container resonance crossing the impact timescale as the eigenvalues $k_m\propto j_{1,m}/b$ move with $b$. The wall condition is therefore a real effect for a small or moderately sized bath, and an irrelevant one at $b=6$; neither the resolvable-dimension diagnostics above nor a convergence study in $b$ itself have been repeated at these smaller radii, so the numbers are a gate, not a validated sweep.
...
pressure truncation at $M=L=60$, the contact time is identical to six significant figures
at every one of $N=2,3,6,10,16$ --- while the solved pressure
...
steps removes it: $\We=0.3$ now runs in $24$ s, and the contact time becomes monotone
decreasing in $\We$ across the range tested, $\We=0.0646$ to $3.0$, as it should be. The complementary failure is also
...
\emph{The contact time, reported as a span.} Every contact time in earlier revisions of this document, and in the repository's README, was computed as the interval from the first contact step to the last. That is wrong, and by more than a rounding: a single impact does not produce a single contact interval. The stepper detaches and re-attaches several times, because the guard that forces one advancing free-flight step after contact ends is followed by immediate re-detection of onset at the next step, so the ``span'' silently includes both that chatter and any genuine free-flight excursion between re-contacts. At the reference case the three candidate definitions give $3.629$ (first interval), $3.815$ (sum over intervals) and $4.262$ (span), a spread of $17\%$. The reported metric is now the first interval's duration, which is the only one measuring a single physical event; the sum remains appropriate for a dissipation budget. Two consequences are worth recording. The span was not merely imprecise but non-monotone in $\We$, whereas the corrected metric decreases monotonically as it should, so the error was masking a diagnostic. And the $N$-independence claim survives the correction unchanged --- the contact time is identical to six significant figures across $N=2,\dots,16$ under either definition --- so nothing built on that claim is affected.

\emph{The case against a constrained no-flux basis for a pinned bath.} \S\ref{subsubsec:wall} argued in an earlier revision that the no-flux set cannot usefully represent a pinned surface, on the strength of a measured wall slope falling six-fold short of the true value at $M=640$. The diagnostic was measuring its own grid: a finite difference across the last interval of a fixed grid, taken inside a boundary layer of width $\sim b/k_M$, returns a number proportional to $\Delta r\,k_M$, and refining the grid at fixed $M$ drives it to zero. Computed analytically the truncation error falls as $M^{-3/2}$ and the wall slope is recovered at every $r<b$. The conclusion is reversed accordingly: the constrained route is the recommended one. The multiplier algebra given alongside it was also wrong --- the constraint force was inserted with the wrong modal normalization, giving a denominator $D=\sum_mJ_0(k_mb)^2\kappa_m/F_m$ whose $m=0$ term is $0/0$ and whose quoted values were neither the sum the formula defines nor the correct one. Treating the multiplier as the line force it physically is gives \eqref{eq:route-b-multiplier}, in which the $J_0(k_mb)$ factors cancel. Separately, the implemented pinned option was recommended on the ground that its only failure was a quantified violation of no-flux; that understated it, since the true cost is loss of exact volume conservation, at four times the droplet's volume.
