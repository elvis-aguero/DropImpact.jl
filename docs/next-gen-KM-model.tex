...
Pointwise over the whole trajectory, the mean absolute deviation is $0.0104$ for the
droplet width against DNS (on a range of $0.974$--$1.117$), $0.1345$ for the position of
the droplet's top and $0.1055$ for its bottom against experiment (on ranges of $1.5$ and
$1.36$ respectively). The trajectory is physically bounded and rebounds: the centre of
mass enters contact at $v=-1.047$ and leaves at $v=+0.314$.
...
Two practical limitations are recorded because they bound what can currently be validated.
The cost per step is not uniform in $\We$, and not monotone in it either: timed at
$M=L=60$, the wall clock is $48,\,27,\,555,\,23,\,24$ seconds at
$\We=0.0121,\,0.0646,\,0.3,\,1.0958,\,3.0$, so $\We\approx0.3$ costs some twenty times more
per step ($597$ ms against $31$--$36$ ms) than either of its neighbours. This is a
localised defect rather than a trend --- most plausibly the feasibility band search
failing to bracket efficiently at that particular state --- and it is not diagnosed here.
Separately, the $\We=0.0121$ run does not terminate contact within the integration window
$\tau_{\text{end}}=14$, so its reported contact time is truncated rather than measured.
Both prevent a clean sweep across the full published $\We$ range for the time being.
