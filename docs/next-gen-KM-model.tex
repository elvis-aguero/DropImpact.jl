...
A separate practical limitation, recorded because it bounds what can currently be
validated: at low Weber number the scheme becomes pathologically slow, with a
$\We\approx0.012$ run failing to complete in many times the wall-clock of a $\We\approx1$
run. The cause is not diagnosed here, and it prevents a sweep across the full published
$\We$ range for the time being.
